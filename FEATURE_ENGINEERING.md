...
Managed by `ProductDataset` in `src/data/dataset.py`. The label mapping is loaded from `data_local/processed/data_v1/label_map.json`, generated by `scripts/prepare_dataset.py` via `attach_label_map()` in `src/data/prepare.py`.
...
All additional fields from `data_local/raw_extracted/data_v1/metadata.csv` are excluded from training features:
